...
class EulerJson:
    ...
    def __init__(self,keypoints_data_path):
        ...
        self.__points_con_relation = [[11,13,15,33],[12,14,16,34],[23,25,27],[24,26,28],[36,35,37]]
        self.__init_coordinate_matrix = self.get_init_coordinate_matrix()
    ...
    def get_init_coordinate_matrix(self):
    ...
    def dict_to_np(self,pose_data_list):
        for i in  range(len(pose_data_list)):
            for j in range(len(pose_data_list[i])):
                pose_data_list[i][j] = np.array([
                    pose_data_list[i][j]['x'],
                    pose_data_list[i][j]['y'],
                    pose_data_list[i][j]['z']
                ])
    ...
    def creat_euler_json(self):
        ...
        self.dict_to_np(pose_data_list)
        for i in range(len(pose_data_list)):
            one_frame_posedata = pose_data_list[i]
            one_frame_euler_angle = []
            for j in range(len(self.__points_con_relation)):
                pose_list = []
                for item in self.__points_con_relation[j]:
                    pose_list.append(one_frame_posedata[item])
                temp_coordinate_matrix = self.__init_coordinate_matrix[j]
                
                rotation_matrix_list = local_vector.rotation_matrix_list(pose_list,temp_coordinate_matrix)
                euler_angle_list = local_vector.get_euler_list(rotation_matrix_list)
                for item in euler_angle_list:
                    one_frame_euler_angle.append(item)

            self.euler_data_list.append(one_frame_euler_angle)
